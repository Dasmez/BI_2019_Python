...
parser.add_argument('--min_length', type=int, default=1, help='non zero length')
parser.add_argument('--keep_filtered', action='store_true', help='stores failed reads')
parser.add_argument('--gc_bounds', nargs='*', type=int, help='no more than 2 values')
parser.add_argument('fastq')
parser.add_argument('--output_base_name', type=str, help='prefix for naming files')
            ...
            with open(name_failed, 'w') as ouf_failed:
                for line in file:
                    ...
                    if args.gc_bounds:
                        ...
                        if gc_content(sequence) < gc_min or gc_content(sequence) > gc_max:
                            if args.keep_filtered:
                                ouf_failed.write(id)
                                ouf_failed.write(line)
                                ouf_failed.write(sense)
