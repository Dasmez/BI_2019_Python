...
numbers_1000 = {x: i for x in range(1, 1001) for i in range(1, 10) if x % i == 0}
print(numbers_1000)
numbers_9 = [max([i for i in range(1, 10) if x % i == 0]) for x in range(1, 1001)]
print(numbers_9)

# 5*. Список всех чисел от 1 до 1000, не имеющих делителей среди чисел от 2 до 9.
numbers_simple = [x for x in range(1, 1001) if len([i for i in range(1, 10) if x % i == 0]) == 1]
